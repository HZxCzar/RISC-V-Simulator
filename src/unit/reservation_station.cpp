...
void ReservationStation::CBD() {
  ...
  for (int pos = 0; pos < 8; ++pos) {
    if (cd_bus_->info.busy(pos) && cd_bus_->info[pos].type_ == BusType::WriteBack) {
...
void ReservationStation::Flush(State *current_state) {
  // std::cerr<<"ReservationStation::Flush\n";
...
void ReservationStation::Execute(State *current_state, State *next_state) {
  for (int pos = 0; pos < 32; ++pos) {
    if (rss_info.busy(pos) && rss_info[pos].q1_ == -1 && rss_info[pos].q2_ == -1) {
