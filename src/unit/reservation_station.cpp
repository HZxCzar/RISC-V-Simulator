...
void ReservationStation::Execute(State *current_state, State *next_state) {
  for(int pos=0;pos<=rss_info.get_pointer();++pos){
    if(rss_info[pos].q1_==-1 && rss_info[pos].q2_==-1){
      ...
      else if(ins.op_type_==OpType::LOAD || ins.op_type_==OpType::STORE){
        alu_info.lhs_=rss_info[pos].v1_;
